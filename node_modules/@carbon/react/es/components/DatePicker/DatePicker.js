...
function updateClassNames(calendar, prefix, isFluid = false) {
	...
	if (calendarContainer && daysContainer) {
		...
		calendarContainer.classList.toggle(`${prefix}--date-picker__calendar--fluid`, isFluid);
	...
	useEffect(() => {
      ...
      <polygon points="5,8 10,3 10.7,3.7 6.4,8 10.7,12.3 10,13 "/>
		...
		const handleInputFieldKeyDown = (event) => {
			...
			if (match(event, Tab)) {
				if (!event.shiftKey && datePickerType === "range" && event.target === startInputField.current && calendar.selectedDates.length === 1 && endInputField.current && !endInputField.current.disabled) {
